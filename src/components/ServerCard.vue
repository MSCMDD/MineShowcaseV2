    ...
    <div class="modal oreui-card" style="position: relative">
        ...
        <div class="server-content" style="padding-bottom: 100px">
            <div class="server-header">
                <h3>{{ server.name }}</h3>
                <span v-if="server.auth_mode==='OFFICIAL'" class="server-status online"><i class="fas fa-circle"></i>正版</span>
                <span v-if="server.auth_mode==='OFFLINE'" class="server-status offline"><i class="fas fa-circle"></i>离线登录</span>
                <span v-if="server.auth_mode==='YGGDRASIL'" class="server-status third-party"><i class="fas fa-circle"></i>第三方登录</span>
            </div>

            <div class="server-meta">
                <div class="meta-item">
                    <span>延迟</span>
                    <span v-if="validate(server.status)">{{ parseInt(server.status.delay) }}</span>
                    <span v-else>(离线)</span>
                </div>
                <div class="meta-item player-count">
                    <span>玩家</span>
                    <span v-if="validate(server.status)">{{ server.status.players.online }}/{{ server.status.players.max }}</span>
                    <span v-else>(离线)</span>
                </div>
                <div class="meta-item">
                    <span>IP</span>
        ...
        <div class="modal_button_area" style="position: absolute;bottom:0;left:0;right:0">
            <a :href=server.link target="_blank" class="oreui-button modal_btn oreui-important-btn">前往官网</a>
            <a :href="'server-info.html?id='+server.id" target="_self" class="oreui-button modal_btn oreui-btn-normal">查看详情</a>
        </div>
    </div>
</template>
