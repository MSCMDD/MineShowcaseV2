...
function refresh(attempt) {
    document.getElementById("title").innerHTML="服务器列表 | MSCPO"
    ...
    let params = `limit=${amountPerPage}&is_member=${!member}&seed=${seed}&offset=${
    ...
    if (keyword.length > 0) {
        $.ajax({
            url: `https://mscpoapi.crashvibe.cn/v1/search?query=${keyword}&` + params,
            method: "GET",
            timeout: 0,
        })
    ...
    $.ajax({
        url: "https://mscpoapi.crashvibe.cn/v1/servers?" + params,
        method: "GET",
        timeout: 0,
    })
        .error(() => refresh(attempt + 1))
...
<template>
    <Page>
        <main class="server-list-container">
            <div class="container">
                <Router :path="['home', 'servers']"></Router>
                ...
                <div class="filter-section oreui-card oreui-dark-card">
                    ...
                    <div class="filter-grid">
                        ...
                        <div class="filter-group">
                            <select
                                id="version-filter"
                                class="filter-select dropdown_label"
                    ...
                    <div style="flex: 1 1 0; text-align: right">
                        <button
                            class="oreui-button oreui-middle-btn oreui-important-btn"
                            id="refresh-btn"
                            @click="reset"
                        >
                            刷新列表
                ...
                <div class="no-results">
                    <div class="oreui-dark-card oreui-card" style="padding: 10px">
                        <p id="list-info">
                            当前为第 {{ currentPage }}/{{ pages }}页({{ amountPerPage }}条/页)
                        </p>

                        <button
                            @click="prev"
                            id="prev-page"
                            class="oreui-button oreui-small-btn oreui-important-btn"
                        >
                            上一页
                        </button>
                        <button
