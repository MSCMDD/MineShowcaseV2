...
function refresh() {
    ...
    $.ajax({
        ...
        document.getElementById("title").innerHTML = "服务器详情: " + response.name + " | MSCPO"
        ...
        <main class="server-detail">
            ...
            <div class="server-info-header">
                <div style="flex: 1 1 0" id="server-desc">
                    ...
                    <div style="margin-top: 15px">
                        <h2 class="ant-typography">{{ server.name }}
                            <span v-if="server.is_member" style="display: inline-block" class="server-badge member"><i class="fas fa-check"></i>成员服</span>
                        </h2>
                ...
                <div style="flex: 0 1 0;">
                    <a :href="server.link" target="_blank">
                        <button class="oreui-button oreui-small-btn oreui-important-btn">官网</button>
                    </a>

                </div>
                <div style="flex: 0 1 0;">
                    <a @click="copy(`${server.ip}`,'已复制到剪贴板！')" target="_blank">
                        <button class="oreui-button oreui-small-btn oreui-btn-normal">复制IP</button>
                    </a>
                </div>
            </div>
            ...
            <div class="box-container">
                <div class="box box-a oreui-dark-card oreui-card" style="">
                    ...
                    <div :hidden="page!=='desc'" id="markdown-section" class="markdown-section"></div>
                ...
                <div class="box box-b oreui-dark-card oreui-card" style="text-align: left">
                    ...
                    <div class="filter-grid" style="padding: 15px">
                        <div class="filter-group">
                            <h3>| 基本信息</h3>
                            <p>服务器IP: <a class="text-secondary" @click="copy(server.ip,'复制成功!')">{{ server.ip }} [点击复制]</a></p>
                            <p>服务器链接: <a class="text-secondary" :href="server.link">[点击前往]</a></p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="filter-group" v-if="online">
                                <p>服务器版本: <span class="text-secondary">{{ server.status.version }}</span></p>
                            </div>

                            <div class="filter-group" v-if="online">
                                <p>在线人数: <span class="text-secondary">{{ server.status.players.online }} / {{ server.status.players.max }}</span></p>
                            </div>

                            <div class="filter-group" v-if="online">
                                <p>延迟(MSCPO服务器测速): <span class="text-secondary">{{ parseInt(server.status.delay) }} ms</span></p>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="filter-group" v-if="online" style="background-color: #222222;padding: 10px;margin-top: 20px">
                                <span class="text-secondary" v-html=" server.status.motd['html'].replaceAll('\n','<br><div style=\'margin:5px\'></div>')"></span>
                            </div>
